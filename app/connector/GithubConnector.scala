...
import model.{Contributor, ErrorResponse, File, RepoContent, RepoData, SearchReposModel, SearchUsersModel, UserInfoModel}
...
class GithubConnector  @Inject() (ws : WSClient){
  ...
  def getUsersRepo(username : String)(implicit ec : ExecutionContext): Future [Either[ErrorResponse,List[RepoData]]]  ={
    ws.url(s"https://api.github.com/users/$username/repos").addHttpHeaders(("Authorization", "Basic ZDZmODg4MmM4YjRlMjc5MjAyM2ZiYmNkNjA1Y2Y5ZTYxYjI5MjIxNA=="))
      .get.map{data => Right(data.json.as[List[RepoData]])}
  }
  ...
  def getUsersInfo(username : String)(implicit ec : ExecutionContext): Future [Either[ErrorResponse,UserInfoModel]] = {
    ws.url(s"https://api.github.com/users/$username").addHttpHeaders(("Authorization", "Basic ZDZmODg4MmM4YjRlMjc5MjAyM2ZiYmNkNjA1Y2Y5ZTYxYjI5MjIxNA=="))
      .get.map{data=> Right(data.json.as[UserInfoModel])}
  }
...
ws.url(url).addHttpHeaders(("Authorization", "Basic ZDZmODg4MmM4YjRlMjc5MjAyM2ZiYmNkNjA1Y2Y5ZTYxYjI5MjIxNA=="))
    .get.map{data => Right(data.json.as[List[Contributor]])}
  }
  ...
  def getRepoContent(username: String, repoName : String )(implicit  ec : ExecutionContext) : Future [Either[ErrorResponse,List[RepoContent]]] ={
    ws.url(s"https://api.github.com/repos/$username/$repoName/contents").addHttpHeaders(("Authorization", "Basic ZDZmODg4MmM4YjRlMjc5MjAyM2ZiYmNkNjA1Y2Y5ZTYxYjI5MjIxNA=="))
      .get.map{data => Right(data.json.as[List[RepoContent]])}
  }
  ...
  def getFile(username : String,repoName : String, path : String )(implicit ec : ExecutionContext) : Future [Either[ErrorResponse,File]] = {
    ws.url(s"https://api.github.com/repos/$username/$repoName/contents/$path").addHttpHeaders(("Authorization", "Basic ZDZmODg4MmM4YjRlMjc5MjAyM2ZiYmNkNjA1Y2Y5ZTYxYjI5MjIxNA=="))
      .get.map{data => Right(data.json.as[File])}
  }
  ...
  def searchUsers (query : String)(implicit ec : ExecutionContext) : Future [Either[ErrorResponse,SearchUsersModel]] = {
    ws.url(s"https://api.github.com/search/users?q=$query").addHttpHeaders(("Authorization", "Basic ZDZmODg4MmM4YjRlMjc5MjAyM2ZiYmNkNjA1Y2Y5ZTYxYjI5MjIxNA=="))
      .get.map{data => Right(data.json.as[SearchUsersModel])}
  }
  ...
  def searchRepos (query : String)(implicit ec : ExecutionContext) : Future [Either[ErrorResponse,SearchReposModel]] = {
    ws.url(s"https://api.github.com/search/repositories?q=$query").addHttpHeaders(("Authorization", "Basic ZDZmODg4MmM4YjRlMjc5MjAyM2ZiYmNkNjA1Y2Y5ZTYxYjI5MjIxNA=="))
      .get.map{data => Right(data.json.as[SearchReposModel])}
  }
